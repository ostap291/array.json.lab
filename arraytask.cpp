#include "arraytask.h"

// Ñîðòóâàííÿ áóëüáàøêîþ çà çðîñòàííÿì (ASC)
void ArrayTask::sortAsc(vector<int>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (arr[j] > arr[j + 1]) {
                int temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
            }
        }
    }
}

// Ñîðòóâàííÿ áóëüáàøêîþ çà ñïàäàííÿì (DESC)
void ArrayTask::sortDesc(vector<int>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (arr[j] < arr[j + 1]) {
                int temp = arr[j];
                arr[j] = arr[j + 1];
                arr[j + 1] = temp;
            }
        }
    }
}

void ArrayTask::printVector(const vector<int>& vec, const string& name) {
    cout << name << ": [ ";
    for (int i = 0; i < vec.size(); i++) {
        cout << vec[i] << " ";
    }
    cout << "]\n";
}

void ArrayTask::saveToJson(int S2, int S3, int duplicatesCount, const vector<int>& V1, const vector<int>& V2) {
    json j;
    j["S2"] = S2;
    j["S3"] = S3;
    j["S3_minus_S2"] = S3 - S2;
    j["duplicates_count"] = duplicatesCount;
    j["V1"] = V1;
    j["V2"] = V2;

    ofstream jsonFile("results.json");
    if (jsonFile.is_open()) {
        jsonFile << j.dump(4);
        jsonFile.close();
        cout << "\nData succesfully saved to results.json\n";
    }
    else {
        cout << "\nError creating JSON file!\n";
    }
}

void ArrayTask::run() {
    srand(time(0));

    int size;
    cout << "Enter array size: ";
    cin >> size;

    if (size <= 0) {
        cout << "Size of array can't be under zero!\n";
        return;
    }

    vector<int> V1(size);
    for (int i = 0; i < size; i++) {
        V1[i] = rand() % 20; // Íåâåëèêèé ä³àïàçîí äëÿ ãåíåðàö³¿ ïîâòîð³â
    }

    printVector(V1, "Initial array V1");

    int n;
    cout << "Enter n (for dividing into groups): ";
    cin >> n;

    ofstream txtFile("e.txt");
    if (txtFile.is_open()) {
        int groupNum = 1;
        for (int i = 0; i < size; i += n) {
            int sum = 0;
            for (int j = i; j < i + n && j < size; j++) {
                sum += V1[j];
            }
            txtFile << "S" << groupNum << " = " << sum << "\n";
            groupNum++;
        }
        txtFile.close();
        cout << "Group sums saved to e.txt\n\n";
    }

    // Ñòâîðþºìî êîï³þ òà ñîðòóºìî çà çðîñòàííÿì
    vector<int> V1_asc = V1;
    sortAsc(V1_asc);

    int S2 = 0;
    for (int i = 0; i < V1_asc.size(); i++) {
        S2 += V1_asc[i];
    }
    cout << "Sum of all V1 elements (S2): " << S2 << "\n";

    // Ñòâîðþºìî êîï³þ òà ñîðòóºìî çà ñïàäàííÿì
    vector<int> V1_desc = V1;
    sortDesc(V1_desc);

    int sumDesc = 0;
    for (int i = 0; i < V1_desc.size(); i++) {
        sumDesc += V1_desc[i];
    }
    cout << "Sum of elements in descending order: " << sumDesc << " (Should match S2)\n";

    vector<int> V2;
    vector<int> deletedElements;

    if (V1_asc.size() > 0) {
        V2.push_back(V1_asc[0]);
        for (int i = 1; i < V1_asc.size(); i++) {
            if (V1_asc[i] != V1_asc[i - 1]) {
                V2.push_back(V1_asc[i]);
            }
            else {
                deletedElements.push_back(V1_asc[i]);
            }
        }
    }

    int S3 = 0;
    for (int i = 0; i < V2.size(); i++) {
        S3 += V2[i];
    }
    cout << "Sum of unique V2 elements (S3): " << S3 << "\n";
    cout << "Difference of sums (S3 - S2): " << (S3 - S2) << "\n";

    int duplicatesCount = deletedElements.size();
    cout << "Number of duplicate elements: " << duplicatesCount << "\n";

    saveToJson(S2, S3, duplicatesCount, V1_asc, V2);

    printVector(deletedElements, "Deleted elements |V1 - V2|");
}